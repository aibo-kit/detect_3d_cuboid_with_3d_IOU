...
void detect_3d_cuboid::set_cam_pose(const Matrix4d &transToWolrd)  //给cam_pose的各个值赋值
{
...
double box3d_iou(const cuboid *sample, const cuboid *ground)
{
    ...
    Eigen::Matrix<double, 3, 8> sam_corner_3d = sample->box_corners_3d_world;    //立方体顶点
    Eigen::Matrix<double, 3, 8> gt_corner_3d = ground->box_corners_3d_world;
    for (size_t i = 0; i < 4; i++)
    {
      point_t sam_top_points(sam_corner_3d(0,i),sam_corner_3d(1,i));   //面的2d坐标
                                     ...
                                     const Eigen::MatrixXd &cub_pose_Twc, std::vector<ObjectSet> &all_object_cuboids, std::vector<ObjectSet> &object_cuboid_after_3d_iou)
{
	...
	if (rgb_img.channels() == 3)
        cv::cvtColor(rgb_img, gray_img, CV_BGR2GRAY); //rgb into gray image
	else
		gray_img = rgb_img;
    ...
    int num_2d_objs = obj_bbox_coors.rows();    //返回行数，此处为1
    ...
    Vector4d ground_plane_sensor = cam_pose.transToWolrd.transpose() * ground_plane_world;  //地面在相机坐标系
    cout<<"ground_plane_sensor = "<<ground_plane_sensor[0]<<" "<<ground_plane_sensor[1]<<" "<<ground_plane_sensor[2]<<" "<<ground_plane_sensor[3]<<endl;

    //       int object_id=1;  即一张图里只有1个物体
	for (int object_id = 0; object_id < num_2d_objs; object_id++)
	{
        ...
        int down_y_raw = top_y_raw + obj_height_raw;    //为什么不是top_y_raw - obj_height_raw

		std::vector<int> down_expand_sample_all;
        down_expand_sample_all.push_back(0);     //先 push 一个 0，如果要扩大采样，就继续 push
        ...
        for(int t=0; t<obj_yaw_samples.size();t++)
        std::cout<<"obj_yaw_samples: "<<obj_yaw_samples[t]<<std::endl;
        //linespace()函数从 a 到 b 以步长 c 产生采样 d for object yaw
        ...
        // 	    int sample_down_expan_id=1;      不采样的时候 down_expand_sample_all.size()就是 1
		for (int sample_down_expan_id = 0; sample_down_expan_id < down_expand_sample_all.size(); sample_down_expan_id++)
		{
            int down_expand_sample = down_expand_sample_all[sample_down_expan_id];   //不采样时就是 0
			int obj_height_expan = obj_height_raw + down_expand_sample;
			int down_y_expan = top_y_raw + obj_height_expan;
            double obj_diaglength_expan = sqrt(obj_width_raw * obj_width_raw + obj_height_expan * obj_height_expan); //对角线
            ...
            for (int edge_id = 0; edge_id < all_lines_raw.rows(); edge_id++)      //检查左上和右下点是否在2d框内
            ...
            for (int i = 0; i < all_lines_merge_inobj.rows(); i++)        //得到了每个线段的角度和中点坐标
			{
			...
			{
                cam_roll_samples.push_back(cam_pose_raw.euler_angle(0));         //不sample roll和pitch的话就直接取相机当前的roll和pitch
				...
				for (int cam_pitch_id = 0; cam_pitch_id < cam_pitch_samples.size(); cam_pitch_id++)
					for (int obj_yaw_id = 0; obj_yaw_id < obj_yaw_samples.size(); obj_yaw_id++)
					{
						if (whether_sample_cam_roll_pitch)
						{
                            ...
                            ground_plane_sensor = cam_pose.transToWolrd.transpose() * ground_plane_world; //更新相机坐标系下的地面
						}
...
//                                                std::cout<<"already get VP = "<<vp_1[0]<<" "<<vp_1[1]
//                                                           <<"VP2 = "<<vp_2[0]<<" "<<vp_2[1]
//                                                           <<"VP3 = "<<vp_3[0]<<" "<<vp_3[1]<<std::endl;
						...
						for (int sample_top_pt_id = 0; sample_top_pt_id < sample_top_pts.cols(); sample_top_pt_id++)
						{
							...
							for (int config_id = 1; config_id < 3; config_id++) // configuration one or two of matlab version
							{
                                ...
                                MatrixXd box_corners_2d_float(2, 8);      //8个点坐标存储在这里
								...
								if (config_id == 1)
								{
                                    visible_edge_pt_ids.resize(9, 2);     //情形1有9条可见边
								...
								{
                                    visible_edge_pt_ids.resize(7, 2);      //情形2有7条可见边
			...
			for (int box_id = 0; box_id < good_proposal_ids.size(); box_id++)
			{
				...
				// if(box_id == 0)
				// {
				// 	std::cout << "2d corner" << all_box_corners_2d_one_objH.block(2 * raw_cube_ind, 0, 2, 8)<< std::endl;
				// 	std::cout << "config" <<all_configs_error_one_objH.row(raw_cube_ind).head<3>()<< std::endl;
				// 	std::cout << "ground_plane_sensor" << ground_plane_sensor << std::endl;
				// std::cout << "cam_pose.invK" << cam_pose.invK << std::endl;
				// std::cout << "cam_pose.transToWolrd" << cam_pose.transToWolrd << std::endl;
				// std::cout << "cam_pose.projectionMatrix" << cam_pose.projectionMatrix << std::endl;
				...
				if (whether_sample_cam_roll_pitch)
				{
                    sample_obj->camera_roll_delta = all_configs_error_one_objH(raw_cube_ind, 7) - cam_pose_raw.euler_angle(0); //为什么减去
                ...
                raw_obj_proposals.push_back(sample_obj);  //1张图片的一个物体的所有立方体的数据，误差都在里面了,还未筛选
			}
		} // end of differnet object height sampling

		// %finally rank all proposals. [normalized_error   skew_error]
        //max_cuboid_num自己调整，此处只要最好的一个，可以扩大范围后，在小范围2次筛选更符合新标准的立方体
        ...
        std::cout<<"actual_cuboid_num_small = "<< actual_cuboid_num_small<<" and max_cuboid_num is = "<<max_cuboid_num<<endl;
        std::cout<<" the Number of cuboid = "<< raw_obj_proposals.size()<<std::endl;
		...
		for (int box_id = 0; box_id < raw_obj_proposals.size(); box_id++)
		{
            ...
            all_combined_score(box_id) = new_combined_error; //得到所有立方体的评分，存入all_combined_score里面,序号很重要
		}
...
//        for(int k = 0; k < sort_idx_small.size(); k++)
//        {
//          std::cout<<" the sort_idx_small = "<< sort_idx_small[k]<<std::endl;
//        }
        ...
        //start using 3d_IOU to select best cuboid
        std::cout<<" number of chosed cuboid = "<<all_object_cuboids[object_id].size()<<std::endl; //此处为 15



        cuboid *ground_truth = new cuboid();;
        ground_truth->pos = cub_pose_Twc.block<1,3>(0,0);
        cout<<" ground_truth_pos : "<<ground_truth->pos(0)<<endl;
        ground_truth->rotY = cub_pose_Twc(0,3);
        ground_truth->scale = cub_pose_Twc.block<1,3>(0,4);

        Matrix3Xd cube_corner_world = compute3D_BoxCorner(*ground_truth);
        ground_truth->box_corners_3d_world = cube_corner_world;
        cout<< "ground truth corner world = "<< ground_truth->box_corners_3d_world.cols()<<endl;
        cout<< "all_object_cuboids_size = "<<all_object_cuboids[0].size()<<endl;


        VectorXd all_3d_iou(all_object_cuboids[object_id].size());
        for(int i = 0; i < all_object_cuboids[0].size(); i++)
        {
            double iou = box3d_iou(all_object_cuboids[0][i], ground_truth);
            //std::cout<<"the 3d_IoU is = "<< iou <<std::endl;
            all_3d_iou(i) = iou;
        }
            VectorXd::Index index_of_max, index_of_max_;  //index_of_max is the position of maximum
            double max_3d_iou = all_3d_iou.maxCoeff(&index_of_max,&index_of_max_);
            cout<<"the max 3d iou = "<< max_3d_iou <<" and it's positoin =  "<<index_of_max<<endl;

            object_cuboid_after_3d_iou.resize(num_2d_objs);

            object_cuboid_after_3d_iou[object_id].push_back( all_object_cuboids[object_id][index_of_max]);
            cout<<"after 3d iou the rotY = "<<object_cuboid_after_3d_iou[object_id][0]->rotY<<endl;
	...
	if (whether_plot_final_images || whether_save_final_images)
	{
        ...
        for (int object_id = 0; object_id < object_cuboid_after_3d_iou.size(); object_id++)
            if (object_cuboid_after_3d_iou[object_id].size() > 0)
			{
                plot_image_with_cuboid(frame_all_cubes_img, object_cuboid_after_3d_iou[object_id][0]);
			}
